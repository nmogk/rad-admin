...
function SourceViewModel(name) {
    ...
    $.ajax({
        url: "http://" + window.location.hostname + ":8008/solr/source/select?", // solr-proxy running on port 8008
...
function RefsViewModel(qString) {
    ...
    var self = this;
    self.refsURI = "http://" + window.location.hostname + ":8008/solr/rad/refs?";
