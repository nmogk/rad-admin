...
Optional `rev_author`/`rev_title`/`rev_source` (all `text_html_safe`, single-valued, all in `_text_`) plus `rev_date` (`string`, single-valued, NOT in `_text_`) capture the work being reviewed when a reference is itself a review (issue #20). `rev_source` is free text — typically a URL — and is intentionally not validated against the sources index. `rev_date` is validated server-side against the same ISO 8601 regex as the main `dt` field but is stored as-is with no derived `year` (the reviewed work's date isn't used for relevance boosting). In the admin add/edit modal these inputs are hidden unless `type === 'reviews'` via the `isReview` computed in `public/javascripts/refViewModel.js`. The values are preserved when type changes (so toggling away and back doesn't lose them), but they are cleared by `holdOver()` so "Save and add another" doesn't carry one review's reviewed-work data into the next entry.
...
The edit endpoint (POST `/refs/:id`) is NOT a pure full-replace despite using `client.add()`. After issue #112, it fetches `oldDoc` up front and merges fields from oldDoc when the corresponding body key is missing or `null` — empty string still means "explicit clear" so editors can blank a field via the textbox. The merge is driven by `PRESERVABLE_FIELDS` in `routes/refs.js`. Date fields are handled separately: when `body.date === oldDoc.dt` (or `body.rev_date === oldDoc.rev_date`), validation is skipped and the existing value is preserved verbatim, so legacy non-standard dates that fail `DATE_RGX` don't block edits to other fields. The client-side counterpart is in `public/javascripts/utils.js` datePicker: an "unknown" precision mode displays the raw value in a text input rather than silently dropping it through `<input type="date">`.
